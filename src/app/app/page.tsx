...
export default async function AppPage() {
  ...
  return <main className="data-lab-shell household-dashboard">
    ...
    <section className="dashboard-metric-grid" aria-label="Household overview">
      <article className="dashboard-metric dashboard-metric-glucose"><div className="dashboard-card-heading"><p>Latest glucose</p><span className={latestReading ? "status" : "status status-warning"}>{latestReading ? "Dexcom" : "No reading"}</span></div>{latestReading ? <><strong>{mmol(latestReading.valueMgDl)} <small>mmol/L</small></strong><p>{formatTime(latestReading.recordedAt)}</p></> : <><strong>—</strong><p>{isConnected ? "Refreshing when you open the dashboard." : "Connect Dexcom to begin."}</p></>}<p className="dashboard-refresh-note">Use the refresh control above whenever you want a fresh pull.</p></article>
