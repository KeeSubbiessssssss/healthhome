...
import { dexcomConnections, glucoseReadings, medicationActivityEvents, medicationDoseLogs, medications, prescriptions } from "@/db/schema";
import { db } from "@/lib/db";
import { currentMember, currentUser } from "@/lib/household";
import { DexcomRefresh } from "@/app/app/dexcom-refresh";
import { InjectableDoseSection } from "@/app/app/injectable-dose-section";
...
export default async function AppPage() {
  ...
  const doseLogs = activeScripts.length === 0 ? [] : await db.select({ prescriptionId: medicationDoseLogs.prescriptionId, unitsConsumed: medicationDoseLogs.unitsConsumed, bslMgDl: medicationDoseLogs.bslMgDl, bslSource: medicationDoseLogs.bslSource, createdAt: medicationDoseLogs.createdAt }).from(medicationDoseLogs).where(inArray(medicationDoseLogs.prescriptionId, activeScripts.map((script) => script.prescriptionId))).orderBy(desc(medicationDoseLogs.createdAt)).limit(40);
  const activityBsl = new Map(activity.filter((event) => event.eventType === "dose_consumed").map((event) => [event.id, doseLogs.find((log) => log.prescriptionId === event.prescriptionId && Number(log.unitsConsumed) === Math.abs(Number(event.unitsDelta)) && log.createdAt.getTime() >= event.createdAt.getTime() && log.createdAt.getTime() - event.createdAt.getTime() < 60_000) ?? null]));
  ...
  return <main className="data-lab-shell household-dashboard">
    ...
    <section className="dashboard-metric-grid" aria-label="Household overview">
      <article className="dashboard-metric dashboard-metric-glucose"><DataPanelRive /><div className="dashboard-panel-content"><div className="dashboard-card-heading"><p>Latest glucose</p><span className={latestReading ? "status" : "status status-warning"}>{latestReading ? "Dexcom" : "No reading"}</span></div>{latestReading ? <><strong>{mmol(latestReading.valueMgDl)} <small>mmol/L</small></strong><p>{formatTime(latestReading.recordedAt)}</p></> : <><strong>—</strong><p>{isConnected ? "Refreshing when you open the dashboard." : "Connect Dexcom to begin."}</p></>}<p className="dashboard-refresh-note">Use the refresh control above whenever you want a fresh pull.</p></div></article>
      <article className="dashboard-metric"><DataPanelRive /><div className="dashboard-panel-content"><div className="dashboard-card-heading"><p>Active scripts</p><span className="dashboard-icon">Rx</span></div><strong>{activeScripts.length}</strong><p>{activeScripts.length === 1 ? "medication currently tracked" : "medications currently tracked"}</p><a className="dashboard-text-button" href="/data-lab">Open data lab</a></div></article>
      <article className={refillAttention.length > 0 ? "dashboard-metric dashboard-metric-attention" : "dashboard-metric"}><DataPanelRive /><div className="dashboard-panel-content"><div className="dashboard-card-heading"><p>Refill attention</p><span className={refillAttention.length > 0 ? "status status-warning" : "status"}>{refillAttention.length > 0 ? "Action needed" : "All clear"}</span></div><strong>{refillAttention.length}</strong><p>{refillAttention.length === 1 ? "script is at its refill point" : refillAttention.length > 1 ? "scripts are at their refill point" : "no scripts are at their refill point"}</p><a className="dashboard-text-button" href="#medication-attention">Review medication</a></div></article>
    </section>
    <InjectableDoseSection scripts={activeScripts.filter((script) => script.form === "injection")} />
    <section className="dashboard-columns">
      <article className="dashboard-panel" id="medication-attention"><DataPanelRive /><div className="dashboard-panel-content"><div className="dashboard-panel-heading"><div><p className="eyebrow">Medication</p><h2>{refillAttention.length > 0 ? "Needs attention" : "Current supply"}</h2></div><a href="/data-lab">View all</a></div>{activeScripts.length === 0 ? <div className="dashboard-empty"><h3>No medication scripts yet</h3><p>Add a script in the Preview data lab to begin tracking units, doses, days, and repeats.</p><a className="dashboard-link" href="/data-lab">Add medication</a></div> : <ul className="dashboard-list">{(refillAttention.length > 0 ? refillAttention : activeScripts).slice(0, 4).map((script) => { const needsRefill = refillAttention.some((item) => item.prescriptionId === script.prescriptionId); const injectable = script.form === "injection"; return <li key={script.prescriptionId}><div className={needsRefill ? "dashboard-list-mark dashboard-list-mark-warning" : "dashboard-list-mark"}>{script.form.slice(0, 1).toUpperCase()}</div><div><strong>{script.pharmaceuticalName}</strong><span>{script.streetName ?? script.strength ?? "Medication script"}</span></div><div className="dashboard-list-meta"><b>{injectable ? displayUnits(script.unitsLeft) : script.daysLeft ?? "—"}</b><span>{injectable ? "units left" : "days left"}</span></div><span className={needsRefill ? "status status-warning" : "status"}>{needsRefill ? "Refill due" : `${displayUnits(script.unitsLeft)} left`}</span></li>; })}</ul>}</div></article>
      <article className="dashboard-panel"><DataPanelRive /><div className="dashboard-panel-content"><div className="dashboard-panel-heading"><div><p className="eyebrow">Activity</p><h2>Recent changes</h2></div><span>{activity.length} shown</span></div>{activity.length === 0 ? <div className="dashboard-empty"><h3>No activity recorded yet</h3><p>Medication actions you take in the data lab will appear here.</p></div> : <ol className="dashboard-activity">{activity.map((event) => { const doseLog = activityBsl.get(event.id); return <li key={event.id}><div><strong>{activityLabel(event.eventType)}</strong><span>{scriptNames.get(event.prescriptionId)} · {event.summary}</span></div><small>{formatTime(event.createdAt)}{Number(event.unitsDelta) !== 0 ? ` · ${Number(event.unitsDelta) > 0 ? "+" : ""}${displayUnits(event.unitsDelta)} units` : ""}{doseLog?.bslMgDl ? ` · BSL ${mmol(doseLog.bslMgDl)} mmol/L${doseLog.bslSource === "manual" ? " (manual)" : ""}` : ""}{event.repeatsDelta !== 0 ? ` · ${event.repeatsDelta > 0 ? "+" : ""}${event.repeatsDelta} repeat` : ""}</small></li>; })}</ol>}</div></article>
    </section>
    <section className="dashboard-glucose-status"><DataPanelRive /><div className="dashboard-panel-content"><div><p className="eyebrow">Dexcom status</p><h2>{isConnected ? "Connected and ready to refresh" : "Connection needs attention"}</h2><p>{connection?.lastSyncedAt ? `Last sync completed ${formatDate(connection.lastSyncedAt)}. ${glucoseCount.value} glucose readings are stored in Preview.` : isConnected ? "Dexcom refreshes when you open or return to this dashboard." : "Connect or reconnect Dexcom to bring glucose readings into HealthHome."}</p></div></div></section>
  </main>;
}
