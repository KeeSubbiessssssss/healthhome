...
export const medicationTreatment = pgEnum("medication_treatment", [
  "diabetes",
  "depression_anxiety",
  "blood_pressure",
  "cholesterol",
...
export const medicationActivityEventType = pgEnum(
  "medication_activity_event_type",
  [
    "script_created",
    "script_updated",
    "script_archived",
...
export const dexcomConnectionStatus = pgEnum("dexcom_connection_status", [
  "not_connected",
  "connected",
  "needs_reauth",
  "error",
]);
export const glucoseTrend = pgEnum("glucose_trend", [
  "double_up",
  "single_up",
  "forty_five_up",
  "flat",
...
export const medications = pgTable(
  "medications",
  {
    ...
    treatmentOf: medicationTreatment("treatment_of"),
    treatmentOther: text("treatment_other"),
...
export const prescriptions = pgTable(
  "prescriptions",
  {
    ...
    totalUnitsPerScript: numeric("total_units_per_script", {
      precision: 10,
      scale: 2,
    }),
    unitsPerDose: numeric("units_per_dose", { precision: 10, scale: 2 }),
    dosesPerDay: integer("doses_per_day"),
...
export const medicationActivityEvents = pgTable(
  "medication_activity_events",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    ...
    unitsDelta: numeric("units_delta", { precision: 10, scale: 2 })
      .notNull()
      .default("0"),
...
export const medicationStock = pgTable(
  "medication_stock",
  {
    ...
    openingQuantity: numeric("opening_quantity", { precision: 10, scale: 2 })
      .notNull()
      .default("0"),
    reorderAtQuantity: numeric("reorder_at_quantity", {
      precision: 10,
      scale: 2,
    })
      .notNull()
  ...
  (table) => [
    index("medication_stock_medication_id_idx").on(table.medicationId),
  ],
);
...
export const inventoryEvents = pgTable(
  ...
  (table) => [
    index("inventory_events_stock_occurred_idx").on(
      table.stockId,
      table.occurredAt,
    ),
...
export const dexcomConnections = pgTable(
  "dexcom_connections",
  {
    id: uuid("id").defaultRandom().primaryKey(),
...
export const dexcomOAuthCredentials = pgTable("dexcom_oauth_credentials", {
  connectionId: uuid("connection_id")
    .primaryKey()
    .references(() => dexcomConnections.id, { onDelete: "cascade" }),
...
export const glucoseReadings = pgTable(
  ...
  (table) => [
    uniqueIndex("glucose_readings_source_reading_id_idx").on(
      table.sourceReadingId,
    ),
    index("glucose_readings_connection_recorded_idx").on(
      table.connectionId,
      table.recordedAt,
    ),
  ],
